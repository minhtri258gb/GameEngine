...
#include "ArchitectureEntity.h"


ArchitectureEntity::ArchitectureEntity(string modelname, string modelext, vec3 origin, int angle, float scale)
{
	m_origin = origin;
...
void ArchitectureEntity::update()
{
	m_controller->update(&m_origin, &m_angle);
}

void ArchitectureEntity::render()
{
	m_model->render(m_origin, m_angle, m_scale);
}

void ArchitectureEntity::load(string modelname, string modelext)
{
...
//			&boundingbox.z);
//	boundingbox *= m_scale;
//	m_controller = new RigidController(boundingbox, m_origin, m_angle);
//
//	delete fCFG;
}
